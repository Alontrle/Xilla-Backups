...
public class RemoveFile extends ResponseEventHandler {

    @Override
    public void ResponseEventHandler(MongoDocument document) {
        Document doc = document.getDocument();
        if(doc.getString("type").equalsIgnoreCase("Remove")) {
            ...
            if(storage.getType().equalsIgnoreCase("FTP")) {
                ...
                ftpServer.addObject(new StorageRequest("Delete", fileObject));
            } else if(storage.getType().equalsIgnoreCase("SFTP")) {
                SFTPServer sftpServer = (SFTPServer)storage;
                sftpServer.addObject(new StorageRequest("Delete", fileObject));
            }
        }
    }
}
